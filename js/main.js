	...
	*/
	...
	P2Game.StateB.prototype=
	{
		...
		//creating 10 patients that show up in a random fashion	
			...
			this.people.createMultiple(15,"people",0,false);
        ...
        // Center it in X, and position its top 15 pixels from the top of the world.
			...
			this.aliveText = this.add.text(50, 530, 'Alive: ', this.style);
			this.deadText = this.add.text(170, 530, 'Dead: 0',this.style);
		
			this.keys = this.input.keyboard.createCursorKeys();
		
			this.timer = this.game.time.create(this.game);
			this.resurrect();
			this.game.time.events.repeat(Phaser.Timer.SECOND*this.rnd.integerInRange(3, 5),15, this.resurrect,this );
		},
		...
		Patient: function(colorStatus,hospitalStatus)
		{
			var status,inHospital;
			this.status=colorStatus;
			this.inHospital=hospitalStatus;
		...
		resurrect: function()
		{
		 ...
		 //Get the first not-currently spawned item
			...
			item=new this.Patient(colorStatus,false);
			this.patientArray.push(item);
			this.aliveCount=(this.aliveCount+1)-this.deadCount;
			this.aliveText.text = 'Alive: ' + this.aliveCount;
			//window.alert(this.patientArray.toString());
		...
		{
			//window.alert(this.patientArray[this.people.getChildIndex(patient)].inHospital);
			if(this.patientArray[this.people.getChildIndex(patient)].inHospital==false)
			{
			patient.alive=false;
			
			patient.visible=false;
			this.deadCount=this.deadCount+1;
			this.deadText.text = 'Dead: ' + this.deadCount;
			}
			//window.alert(patient.exists);
			if(this.deadCount==3 || this.deadCount>3)
				{
					this.goToStateC();
				}
		},
			...
			if(this.physics.arcade.overlap(this.rect,item,null,null,this))
				{
					...
					if(Number.isInteger(bedNum)!=false)
					{
						...
						this.patientArray[this.people.getChildIndex(item)].inHospital=true;
	...
	P2Game.StateC.prototype = 
	{
		...
		update: function()
		{
			...
			this.style = { font: "45px Verdana", fill: "red", align: "center" };
			this.text = this.add.text( this.world.centerX, this.world.centerY, "You Killed 3 Patients Doctor, \nthis is UNACCEPTABLE!\n (press the left arrow to play again)", this.style );
			this.text.anchor.setTo( 0.5, 0.0 );
